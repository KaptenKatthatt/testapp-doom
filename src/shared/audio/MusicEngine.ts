  ...
  private generateRampage(): void {
    ...
    const chugPattern = (startBar: number, root: number, bars: number): void => {
      for (let b = 0; b < bars; b++) {
        ...
        // Blast beat: kick every 2, snare every 4
        for (let s = 0; s < 16; s += 2) this.drumPattern.set(off + s, 'kick');
        for (let s = 4; s < 16; s += 4) this.drumPattern.set(off + s, 'snare');
        for (let s = 1; s < 16; s += 2) this.drumPattern.set(off + s, 'hat');
      }
    };

    // Section 1: E minor frenzy (bars 0-7)
    chugPattern(0, 40, 4);
    chugPattern(4, 43, 4);

    // Section 2: F and G aggression (bars 8-15)
    chugPattern(8, 41, 4);  // F
    chugPattern(12, 43, 4); // G

    // Section 3: Breakdown half-time (bars 16-23)
    for (let b = 0; b < 8; b++) {
      const off = (16 + b) * 16;
      ...
      this.drumPattern.set(off + 8, 'kick');
      this.drumPattern.set(off + 12, 'snare');
      for (let s = 2; s < 16; s += 2) this.drumPattern.set(off + s, 'hat');
    }

    // Section 4: Climax full blast (bars 24-31)
    chugPattern(24, 40, 2);
    chugPattern(26, 43, 2);
    chugPattern(28, 41, 2);
    // Final 2 bars: all-out
    const end = 30 * 16;
    for (let s = 0; s < 32; s += 2) {
      this.guitarPattern.push({ step: end + s, note: s < 16 ? 40 : 43, duration: 1, type: 'mute' });
      this.bassPattern.push({ step: end + s, note: s < 16 ? 28 : 31, duration: 1 });
      this.drumPattern.set(end + s, s % 4 === 0 ? 'kick' : s % 4 === 2 ? 'snare' : 'hat');
    }
